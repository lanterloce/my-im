...
const username=ref('demo');const password=ref('123456');const logged=ref(false);const token=ref('');const content=ref('');const messages=ref<any[]>([]);const conversationId=1
async function login(){try{const r=await api.post('/api/users/login',{username:username.value,password:password.value});token.value=r.data.token;logged.value=true;await load()}catch(e){alert('登录失败：请先注册或检查后端')}} 
...
async function send(){if(!content.value.trim())return;await api.post('/api/messages',{conversationId,senderId:1,content:content.value});content.value='';await load()}
onMounted(()=>{})
</script>
